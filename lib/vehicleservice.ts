...
import { commonEmptyResponse } from '@/services/helper';
...
export const getVehicleBookings = async ({
  ...
  from?: string;
  to?: string;
}): Promise<ApiPagination & { data: any[] }> => {
  try {
...
export const getVehicleById = async (id: string): Promise<any | null> => {
  try {
...
export const getBookingById = async (id: string): Promise<any | null> => {
  try {
